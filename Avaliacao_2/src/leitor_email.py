"""
Módulo responsável pelo monitoramento da caixa de entrada, leitura de e-mails de cadastro de clientes e download dos PDFs unificados.
"""
import os
import imaplib
import email
...
class LeitorEmail:
    """
    Classe responsável por conectar à caixa de entrada (IMAP) e monitorar e-mails de cadastro
    ...
    def ler_emails_pendentes(self, marcar_como_lido: bool = True, permitir_simulacao: bool = False) -> list:
        ...
        if self.email_user and self.email_pass and self.imap_server:
            try:
                ...
                # Busca estrita por e-mails NÃO LIDOS (UNSEEN) que contenham "Cadastro" no assunto
                status, messages = mail.search(None, '(UNSEEN SUBJECT "Cadastro")')
